...
"""Обнуление БД"""
# result = mdb.update_one({"_id": "2"}, {"$set": {"number code": [], "letter code": [], "units": [], "currency": []}})  # обнуление данных с курсом
receive = requests.get("https://www.cbr-xml-daily.ru/daily_json.js")
data = json.loads(receive.text)
# mdb.update_one({"_id": "2"}, {"$push": {"number code": "840", "letter code": "USD", "units": "1", "currency":"Доллар США"}})
# mdb.update_one({"_id": "2"}, {"$push": {"number code": "978", "letter code": "EUR", "units": "1", "currency":"Евро"}})
...
#
#
# """ Работа с парсером и работа с текстом (НЕ РАБОЧАЯ ВЕРСИЯ ГАВНАПАРСЕРА)"""
